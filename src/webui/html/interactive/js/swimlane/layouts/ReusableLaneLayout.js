...
function ReusableLaneLayout() {
	...
	var layout = function(data, height) {
		...
		for (var i = 0; i < processes.length; i++) {
			// Get the process and determine its offset
			var process = processes[i];
			var offset = i * processspacing + procmap[process.ID()] * laneheight;
			
			// Cache the process's offset
			cache[process.ID()] = offset;
			
			// Now do the spans for the process
			var spans = process.Spans();
			for (var j = 0; j < spans.length; j++) {
				var span = spans[j];
				var spanid = spans[j].ID();
				cache[span.ID()] = offset + lanemap[spanid] * laneheight;
